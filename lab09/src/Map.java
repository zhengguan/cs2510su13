...
public class Map<T> {
	...
	public <T> void replaceBang(ArrayList<T> is, Predicate<T> p, T t) {
	...
	// EFFECT: replace every element with the result of applying the function 
	// to that element
	public <T> void updateBang(ArrayList<T> is, Fun<T,T> f) {
		updateBangHelper(is, f, is.size());
	}
	
	// WHERE: 0 <= end && end <= is.size();
	// EFFECT: replace every element of is[0, end) with the resut of applying 
	// the functon to the result 
	private <T> void updateBangHelper(ArrayList<T> is, Fun<T,T> f, int end) {
		if (end == 0) {
			return;
		}
		else {
			is.set(end - 1, f.apply(is.get(end - 1)));
			updateBangHelper(is, f, end - 1);
			return;
		}
	}
}
...
class MapExamples {	
	...
	private void reset() {
		...
		this.add1 = new Fun<Integer, Integer>() {
	...
	// tests for method updateBang
	public void testUpdateBang(Tester t) {
		reset();
		map.updateBang(mt, add1);
		t.checkExpect(mt, new ArrayList<Integer>());
		map.updateBang(oddl, add1);
		t.checkExpect(oddl, evenl);
		reset();
	}
}