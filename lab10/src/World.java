...
class SimpleWorld extends World {
	private ArrayList<MyCircle> loc;
...
//		this.c1 = new CircleImage(new Posn(100, 0), 10, new Green());
		this.loc = new ArrayList<MyCircle>();
		this.loc.add(new MyCircle(new Posn(100, 0), CIRCLE_RADIUS, 
				CIRCLE_COLOR));
		this.loc.add(new MyCircle(new Posn(200, 0), CIRCLE_RADIUS, 
				CIRCLE_COLOR));
	}
	
	private SimpleWorld(ArrayList<MyCircle> loc) {
//		this.c1 = c1;
		this.loc = loc;
	}
...
class MyCircleExamples {
	MyCircle c1;
	
	private void reset() {
		c1 = new MyCircle(new Posn(100, 0), 10, new Red());
	}
	
	// tests for method onTick
	public void testOnTick(Tester t) {
		reset();
		t.checkExpect(c1.onTick(100), 
				new MyCircle(new Posn(100, 10), 10, new Red()));
		reset();
	}
}
